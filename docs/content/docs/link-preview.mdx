...
| `onPressError` | `(error: Error) => void` | — | Fires when the default `Linking.openURL` call fails. |
...
The component prefers the largest valid HTTPS image returned by the page. If
that image fails to decode, it falls back to
