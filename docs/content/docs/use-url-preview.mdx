...
network call is made, no cache entry is created. Timeout errors are _not_
cached (they're treated as transient); other server errors are cached for 30
seconds by default to avoid hammering a flaky endpoint. See
[Caching](/docs/caching) to tune those windows.
