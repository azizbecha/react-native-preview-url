...
function useUrlPreview(
  url: string,
  timeoutOrOptions?: number | UseUrlPreviewOptions
): {
  loading: boolean;
...
Pass a number for the original timeout API, or an options object:
...
Custom `fetcher`, `headers`, and `signal` values intentionally bypass the
shared cache and in-flight deduplication, so request-specific credentials and
cancellation cannot leak between consumers.

The hook re-runs whenever `url` or its request options change. Concurrent renders for
the same URL share a single in-flight request — only one network call goes
out, even across many components.

Call `refresh()` to invalidate the current URL and fetch its latest metadata:

```tsx
const { data, refresh } = useUrlPreview(url);

<Button title="Refresh preview" onPress={refresh} />;
```
